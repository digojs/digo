...
export class FileList extends EventEmitter {
     ...
     * 全局任务序号。
     */
    private taskId = beginAsync("(FileList)", this);
    ...
    pipe<T>(processor: string | Processor<T> | (new (options?: T) => FileList) | FileList, options?: T): FileList {
        ...
        if (typeof processor === "string") {
            processor = <FileList | (new (options?: any) => FileList) | Processor<T>>plugin(processor);
        }
        ...
        if (processor.prototype instanceof FileList) {
            return this.pipe(new (<new (options?: any) => FileList>processor)(options));
        }
        ...
        if (processor.length < 4) {
            let waiting = 1; // 需要等待所有 data 事件 + 1 个 end 事件。
            this.on("data", file => {
                waiting++;
                file.load((error, file) => {
                    ...
                    if ((<Processor<T>>processor).length < 3 || error) {
                        done();
                    }
                });
            });
                ...
                file.load((error, file) => {
                    ...
                    try {
                        (<Processor<T>>processor)(file, options, function done() {
                            if ((<Processor<T>>processor).length === 4) {
                                result.add(file);
                            }
                            endAsync(taskId);
                            proc(index + 1);
    ...
    dest(dir?: string | ((name: string, file: File) => string)) {
        ...
        this.on("data", file => {
            waiting++;
            file.save(typeof dir === "function" ? dir(file.destName, file) : resolvePath(dir, file.destName), (error, file) => {
                result.add(file);
                if (--waiting > 0) return;
                result.end();
            });
