...
Builder.prototype._currentRuleIndex = 0;

Builder.prototype._currentRunOnceRuleIndex = 0;
...
Builder.prototype.build = function (dest, clean) {
	...
	if (this.runOnceRules.length > 0) {
		...
		for (var i = this._currentRunOnceRuleIndex; i < this.runOnceRules.length; i++) {
...
Builder.prototype.process = function (path, content, tmpFile) {
	...
	if (!file) {
		...
		// 依次应用每个规则。
		for (var i = this._currentRuleIndex; i < this.rules.length; i++) {
			var rule = this.rules[i];
			
			// 判断指定规则是否可用于当前文件路径。同时获取当前规则的发布结果。
			var dest = rule.match(file.srcPath);
			if (dest) {
