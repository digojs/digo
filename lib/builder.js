...
Builder.prototype.__defineGetter__('destFullPath', function () {
    return Path.resolve(this.destPath);
});

/**
 * 所有绝对路径的根路径。即项目中路径“/”所表示的实际位置。
 */
Builder.prototype.basePath = "";
...
/**
 * 获取指定完整路径的简短路径。如 a/b.jpg
 * @param {String} fullPath 要获取的绝对路径。
 */
 ...
 */
Builder.prototype.buildFile = function (path, content) {
    // 不重复生成同一个文件。
    var file = this.files[path];
    if (file) {
        return file;
    }
    this.files[path] = file = new BuildFile(this, path, content);
    this.processFile(file, content == null);
    return file;
};

/**
 * 根据当前设置的规则处理指定文件。
 * @param {String} file 要处理的文件。
 * @param {Boolean} [save] 指示是否保存文件。
 */
Builder.prototype.processFile = function (file, save) {

    var lastErrorCount = this.errors.length;

    // 依次执行所有适用的规则。
    for (var i = 0; i < this.rules.length; i++) {
        var dest = this.rules[i].match(file.path);
        if (dest !== null) {
            this.processCore(file, this.rules[i], dest);
        }
    }

    // 如果文件处理时未发生错误，则保存文件。
    if (save && this.errors.length === lastErrorCount && file.save()) {
        this.fileCount++;
    }
 ...
 */
Builder.prototype.processCore = function (file, rule, dest) {
    ...
    for (var i = 0; i < rule.processors.length; i++) {
        var options = Object.assign({}, rule.processorOptions[i]);
        if (this.verbose) {
...
Builder.prototype.addFile = function (path, content) {
    var file = this.files[path] = new BuildFile(this, "", content);
    file.path = path;
...
Builder.prototype.watch = function () {
    var builder = this;
    this.watcher = watchDir(this.srcFullPath, function (path) {
        if (builder.ignored(path)) {
            return false;
        }
        ...
        path = builder.getPath(path);
        delete builder.files[path];
        builder.buildFile(path);
    });
    this.info("Watching {0}...", this.srcFullPath);
};
...
Builder.prototype.startServer = function (port) {
    ...
    var server = new HttpServer({
        port: port || 8080,
        physicalPath: this.srcFullPath,
        handlers: this.handlers,
        modules: {
            buildModule: {
                processRequest: function (context) {
                    var path = context.request.path;
                    var file = new BuildFile(builder, path);
                    if (file.exists) {
                        builder.processFile(file);
                        context.response.contentType = server.mimeTypes[Path.extname(file.path)];
...
BuildFile.prototype.__defineGetter__('destFullPath', function () {
    return this.destPath ? Path.join(this.builder.destFullPath, this.destPath) : null;
});
...
BuildFile.prototype.__defineGetter__('exists', function () {
    return this.srcPath && IO.existsFile(this.builder.getFullPath(this.srcPath));
});
...
BuildFile.prototype.load = function (encoding) {
    ...
    try {
        ...
        this.builder.warn("{0}: Cannot Read File: {1}", srcFullPath, e);
        return '';
    }
};
...
BuildFile.prototype.save = function () {
    ...
    if (this._contentUpdated) {
        try {
            ...
            this.builder.error("{0}: Cannot Write File: {1}", destFullPath, e);
        }
        return true;
    }
    ...
    if (srcFullPath !== destFullPath) {
        try {
            ...
            this.builder.error("{0}: Cannot Copy File: {1}", destFullPath, e);
        }
        return true;
    }
...
BuildFile.prototype.resolvePath = function (relativePath) {
    ...
    return (/^\//.test(relativePath) ? Path.join(this.builder.basePath + relativePath).replace(/\\/g, '/').replace(/^\//, '') : Path.join(Path.dirname(this.srcPath), relativePath).replace(/\\/g, '/'));
};
 ...
 * @param {String} [content] 如果是虚拟文件，则手动指定文件的内容。
 */
BuildFile.prototype.addDependency = function (relativePath, content) {
    var file = this.builder.buildFile(this.resolvePath(relativePath), content);
    if (content == null && !file.exists) {
        this.builder.warn("{0}: Reference Not Found: {1}", this.srcFullPath, relativePath);
    }
