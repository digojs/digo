...
Builder.prototype.build = function (dest, clean) {
	...
	this._currentRuleIndex = this.rules.length;
	this._currentRunOnceRuleIndex = this.runOnceRules.length;
...
Builder.prototype.getFile = function (path) {
	var file = this.files[path] || new BuildFile(this, path);
...
Builder.prototype.process = function (path, content) {
	...
	for (var i = 0; i < this.rules.length; i++) {
		...
		// 设置当前已执行的规则索引。
		file.ruleIndex = this._currentRuleIndex = i;
...
Builder.prototype.processFileWithRule = function (file, rule) {
	...
	this.processFileWithRuleCore(file, rule, dest);
 ...
 */
Builder.prototype.processFileWithRuleCore = function (file, rule, dest) {
	
	this.log('> {0} -> {1}', file.path, dest);
 ...
 */
BuildFile.prototype.watch = function (callback) {
	var builder = this;
...
BuildFile.prototype.ruleIndex = 0;
...
BuildFile.prototype.save = function (destBasePath) {
	...
	var destFullPath = Path.resolve(destBasePath || '.', this.path);
...
function createPathFilter(filter) {
	...
	if (filter instanceof RegExp) {
		return function (path, dest) {
			...
			return match ? dest ? dest.replace(/\$(\d+)/g, function (all, n) {
	...
	if (filter instanceof Function) {
		return function (path, dest) {
			...
			return match === true ? dest || path : match === false ? null : match;
		};
	}
