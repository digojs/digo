...
Builder.prototype.build = function (clean) {
	return this.flush(clean, true);
};

/**
 * 根据当前设置的规则生成整个项目。
 * @param {Boolean} [clean] 指示是否在生成前清理文件夹。默认为自动清理。
 * @param {Boolean} [finalFlush=false] 指示本次生成操作是否是最后一次生成操作。
 * @returns {Number} 返回本次生成所保存的文件数。 
 */
Builder.prototype.flush = function (clean, finalFlush) {
...
Builder.prototype.buildFile = function (path) {
	var file = this.processFile(path);
...
Builder.prototype.processFile = function (path, content, isTmpFile) {
	...
	if (!file || isTmpFile) {
		...
		} else if (processed && file.path && this.errors.length === lastErrorCount) {
...
BuildFile.prototype.resolvePath = function (relativePath) {
    return (/^\//.test(relativePath) ? Path.join(this.builder.virtualPath + "/" + relativePath) : Path.join(Path.dirname(this.srcFullPath), relativePath)).replace(/\\/g, '/');
};
