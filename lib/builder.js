 ...
 * 获取或设置当前构建工具的原始路径。
 */
Builder.prototype.srcPath = null;

/**
 * 获取或设置当前构建工具的生成路径。
 */
Builder.prototype.destPath = null;
...
Builder.prototype.reset = function () {
	this._ignoreFilter = null;
	this.runOnceRules.length = this.rules.length = this.ignores.length = this.errors.length = 0;
	this.files = Object.create(null);
};
...
Builder.prototype.__defineGetter__('destFullPath', function () {
	return this.destPath ? Path.resolve(this.srcFullPath, this.destPath) : this.srcFullPath;
});
 ...
 */
Builder.prototype.processFile = function (path, content, isTmpFile, disableLogging) {
	...
	if (!file || isTmpFile) {
		...
		if (!disableLogging && processed && file.path && this.errors.length === lastErrorCount) {
			this.log(file.srcPath === file.path ? "> {0}" : '> {0} -> {1}', file.srcPath, file.path);
		}
...
Builder.prototype.getFullPath = function (path) {
	return Path.resolve(this.srcFullPath, path || '');
};
...
Builder.prototype.processDependency = function (baseFile, path, content) {
	...
	// 存在内容说明是自定义内容。
	if (content != null) {
		return this.processFile(path, content, true, true);
	}
	
	// 否则说明是当前文件的依赖文件。
	var fullPath = this.getFullPath(path);
	if (IO.existsFile(fullPath)) {
		var file = this.processFile(path, content, false);
		baseFile.dependencies.push(file);
		return file;
	}
	
	// 文件不存在输出警告。
	this.warn("{1}: Reference not found: {0}", fullPath, baseFile.srcPath);
 ...
 * @type {String}
 */
BuildFile.prototype.__defineGetter__('destPath', function () {
	return this.path;
});
...
BuildFile.prototype.__defineGetter__('destFullPath', function () {
	return this.destPath ? this.builder.getFullPath(this.destPath) : "";
});
...
BuildFile.prototype.save = function (destBasePath) {
	...
	if (srcFullPath !== destFullPath) {
		if(this.path === this.srcPath){
			this.builder.log('+ {0}', this.srcPath);
		}
...
BuildFile.prototype.resolvePath = function (relativePath) {
	return (/^\//.test(relativePath) ? Path.join(this.builder.virtualPath + "/" + relativePath) : Path.join(Path.dirname(this.srcPath), relativePath)).replace(/\\/g, '/');
};
...
BuildFile.prototype.relativePath = function (path) {
	return Path.relative(Path.dirname(this.srcFullPath), this.builder.getFullPath(path)).replace(/\\/g, '/');
};
