...
Builder.prototype.processFile = function (path, content, isTmpFile) {
    ...
    if (!file) {
        ...
        if (isTmpFile) {
            ...
            if (file.srcPath === file.path) {
                this.log("> {0}", file.srcPath);
            } else {
                this.log('> {0} -> {1}', file.srcPath, file.path);
            }
        }
...
Builder.prototype.addFile = function (path, content) {
    this.log("+ {0}", path);
    return this.files[path] = new BuildFile(this, path, content);
};
...
/**
 * 添加当前文件的依赖项。
 * @param {String} path 依赖项的路径。
 * @param {String} [content] 手动依赖项的内容。
 * @returns {BuildFile} 返回依赖项文件。 
 */
BuildFile.prototype.addDependency = function (path, content) {
    var fullPath = this.builder.getFullPath(path);
    if (IO.existsFile(fullPath)) {
        var file = this.builder.processFile(path, content);
