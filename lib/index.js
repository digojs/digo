...
exports.task = function (taskName, taskAction, subTask) {
	...
	if (typeof exports.tasks[taskName] === 'function') {
		...
		exports.builder = exports.rootBuilder.clone();
		try {
			
			// 启动任务。
			if (!subTask) {
				exports.info("Starting Task {0}", taskName);
			}
			
			return exports.tasks[taskName](taskAction == null ? {} : taskAction);
		} finally {
			
			var fileCount = (exports.builder.fileCount || 0) + (oldBuilder.fileCount || 0);
			
			exports.builder = oldBuilder;

		    exports.builder.fileCount = fileCount;
			
			// 报告错误。
			if (!subTask) {
				var fileCount = exports.builder.fileCount;
				var errorCount = exports.builder.errors.length;
				if (errorCount) {
					exports.builder.error("Task Completed! ({0} files updated, {1} error(s) found)", fileCount, errorCount);
				} else {
					exports.builder.success("Task Success! ({0} files updated)", fileCount, errorCount);
				}
			}
...
function initOptions(options) {
	exports.destPath = options.dest || options.out || options.o;
	exports.logLevel = options.level;
	return options;
};
